  ...
  private int serialize(TrieNode root, BitOutputStream out, int level) {
    ...
    if (0 == level) {
      ...
      root.streamDecendents(level).forEach(node -> {
        ...
        if (null != verbose) {
          verbose.println(String.format("Recursing %s run godparent %s and %s of %s potential children %s", node.getDebugString(), godparent.getDebugString(), children.size(), godchildren.size(), godchildren.keySet()));
        }
  ...
  private long deserialize(TrieNode root, BitInputStream in, int level) {
    ...
    if (0 == level) {
      ...
      root.streamDecendents(level).forEach(node -> {
        ...
        if (null != verbose) {
          verbose.println(String.format("Recursing %s run godparent %s and %s of %s potential children %s", node.getDebugString(), godparent.getDebugString(), numberOfChildren, godchildren.size(), godchildren.keySet()));
        }
        godchildren.forEach((token, godchild) -> {
          try {
            int godchildAdj = godchildCounters.getOrDefault(godchild.getDebugString(), 0);
