...
public class MarkdownPrintStream extends PrintStream implements NotebookOutput {
  ...
  public MarkdownPrintStream(File parentDirectory, String name) throws FileNotFoundException {
    super(new FileOutputStream(parentDirectory));
    this.name = name;
  ...
  public <T> T code(Supplier<T> fn, int maxLog, int framesNo) {
    try {
      ...
      out("Code from [%s:%s](%s#L%s) executed in %.2f seconds: ",
          ...
          Util.pathTo(workingDir.getParentFile(), CodeUtil.findFile(callingFrame)), callingFrame.getLineNumber(), result.obj.seconds());
      out("```java");
  ...
  public File getResourceDir() {
    File etc = new File(this.workingDir.getParentFile(), "etc");
    etc.mkdirs();
    return etc;
  }
